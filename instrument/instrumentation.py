...
def save_as_instrumented_python(instrumented, input_file, name, path):
    ...
    out_path = './out_vis/' + f"code_instrumented.py"
    evalue_dir = os.path.dirname(os.path.abspath(__file__))
    cov_dir = evalue_dir[:-11]
    imports = import_count(path, input_file)
    extra, max_number = max_line(path, input_file, name)
    ...
    with open(out_path, "w") as out_file:
        for num, line in enumerate(buf):
            if 'return' in line and num < max_number + 7 + len(imports):
                count = 0
                for i in line:
                    if i == ' ':
                        count += 1
                ...
                new = ' ' * count + "distances_true = evaluate_condition('*', '*', '*', '*')"
                out_file.write(new)
                out_file.write("\n")
                out_file.write(line[:-1]+", distances_true\n")
            else:
                out_file.write(line)


def max_line(input_path, file, func):
    ffile = open("./code.py", "r")
...
def import_count(path, mod):
    imports = list()
    count = 0
    tree = ast.parse(open(f'code.py').read())
